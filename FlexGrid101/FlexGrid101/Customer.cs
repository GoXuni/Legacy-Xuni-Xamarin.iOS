...
namespace FlexGridSample
{
		...
		IEditableObject
	{
		...
		public static ObservableCollection<Customer> GetCustomerListCollection (int count)
		{
		...
		public static KeyValuePair<int, string>[] GetCountries ()
		{
			return _countries.Select ((p, index) => new KeyValuePair<int, string> (index, p.Key)).ToArray ();
		}
